...
ProductsController.get('', async (req: Request, res: Response) => {
    try {
        ...
        let filter: { where?: { id_brand? : string, name? : {}}, offset?: number, limit?: number, order?: [[string, string]] } = {};
        ...
        for (const index in products) {
            ...
            const options = await OptionsService.findOneOptionBasicByIdProduct(products[index].id_product, ['id_option', 'id_product', 'price', 'memory']);
            if (options) {
